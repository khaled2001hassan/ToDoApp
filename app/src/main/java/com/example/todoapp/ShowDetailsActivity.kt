    ...
    fun initListener(idThis: Int) {
        ...
        upDateTodoButton.setOnClickListener {
            ...
            val todo = Todo(
                ...
                description = todoDescriptionView.editText!!.text.toString(),
                isDone = false,
                date = selectedDay.time.time,
                id = idThis
            )
...
//            onUpdateClick?.onClickUpDate()
    ...
    fun refreshDateView() {
        dateView.text = "${selectedDay.get(Calendar.YEAR)}/${selectedDay.get(Calendar.MONTH)}/${
            selectedDay.get(
